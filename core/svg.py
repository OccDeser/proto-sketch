...
"""Picture Settings"""
def PIC_DPI(): return GLOBAL_OPTIONS.pic['dpi']
def PIC_ZOOM(): return GLOBAL_OPTIONS.pic['zoom']
def PIC_MARGIN(): return GLOBAL_OPTIONS.pic['margin']


"""Actor Settings"""
def ACTOR_MIN_WIDTH(): return GLOBAL_OPTIONS.actor['min_width']
def ACTOR_MIN_SPAN(): return GLOBAL_OPTIONS.actor['min_span']
def ACTOR_MARGIN(): return GLOBAL_OPTIONS.actor['margin']


"""Action Settings"""
def ACTION_MIN_WIDTH(): return GLOBAL_OPTIONS.action['min_width']
def ACTION_MARGIN(): return GLOBAL_OPTIONS.action['margin']


"""Message Settings"""
def MSG_LINE_HEIGHT(): return GLOBAL_OPTIONS.message['line_height']
def MSG_BOTTOM_MARGIN_PIXEL(): return GLOBAL_OPTIONS.message['bottom_margin_pixel']


"""Protocol Settings"""
def PROTO_MARGIN(): return GLOBAL_OPTIONS.protocol['margin']
def END_MARGIN(): return GLOBAL_OPTIONS.protocol['end_margin']
def END_HEIGHT(): return GLOBAL_OPTIONS.protocol['end_height']
def END_WIDTH(): return GLOBAL_OPTIONS.protocol['end_width']
def END_ZOOM(): return GLOBAL_OPTIONS.protocol['end_zoom']
def LINE_WIDTH(): return GLOBAL_OPTIONS.protocol['line_width']
def GRID_SIZE(): return GLOBAL_OPTIONS.protocol['grid_size']


"""folder settings"""
def ARROW_FOLDER(): return GLOBAL_OPTIONS.folder['arrow']
def CACHE_FOLDER(): return GLOBAL_OPTIONS.folder['cache']
...
def draw_action(action, pic: Picture, pixel_size: Tuple[int, int]) -> str:
    ...
    width, height = pixel_size
    pic_width, pic_height = pic.pixel_size
    insert = (floor((width-pic_width)/2), floor((height-pic_height)/2))
    # add image
    png_url = f"data:image/png;base64,{pic.base64}"
    dwg.add(dwg.image(href=png_url, insert=insert, size=pic.pixel_size))
    svg_base64 = base64.b64encode(dwg.tostring().encode()).decode("utf-8")
...
def draw_protocol(proto: Protocol, outfile: str, cache=True):
    ...
    for draw in proto.draws:
        if draw.src == draw.dst:
            ...
            insert = (insert_xpixel, proto_ypixel)
            # add rectange
            # TODO: costomize action style
            dwg.add(dwg.rect(insert=insert, size=pixel_size,
